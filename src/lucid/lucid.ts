...
export class Lucid {
    ...
    selectWalletFromPrivateKey(privateKey: PrivateKey): Lucid {
        ...
        this.wallet = {
            ...
            signTx: async (
                ...
                const txWitnessSetBuilder =
                    C.TransactionWitnessSetBuilder.new();
            ...
            signMessage: async (
                ...
                if (!keyHash || keyHash !== originalKeyHash) {
                    throw new Error(
                        `Cannot sign message for address: ${address}.`
                    );
                }
        ...
        this.wallet = {
            ...
            getUtxosCore: async (): Promise<C.TransactionUnspentOutputs> => {
                ...
                ((await api.getUtxos()) || []).forEach((utxo) => {
                    utxos.add(
                        C.TransactionUnspentOutput.from_bytes(fromHex(utxo))
                    );
                });
                return utxos;
            },
            ...
            signMessage: async (
                ...
                const hexAddress = toHex(
                    C.Address.from_bech32(address).to_bytes()
                );
    ...
    selectWalletFromSeed(
        ...
        this.wallet = {
            ...
            getUtxosCore: async (): Promise<C.TransactionUnspentOutputs> => {
                const coreUtxos = C.TransactionUnspentOutputs.new();
                (await this.utxosAt(paymentCredentialOf(address))).forEach(
                    (utxo) => coreUtxos.add(utxoToCore(utxo))
                );
                return coreUtxos;
            },
            getDelegation: async (): Promise<Delegation> => {
            ...
            signMessage: async (
                ...
                const keyHash =
                    paymentCredential?.hash || stakeCredential?.hash;
